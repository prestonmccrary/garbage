...
def plot_scalars(ax: plt.Axes,
    ...
    if extra:
        evn_steps = extra["Train_EnvstepsSoFar"]['value']

        print(evn_steps[15])

        ax.plot(evn_steps[:15], scalars[data_key]['value'][:15], color=color, label=name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_key", "-d", nargs="+", required=True)
    ...
    if len(args.data_key) < len(args.input_log_files):
        args.data_key = [args.data_key[0] for _ in args.input_log_files]
    ...
    if args.plot_mean_std:
        ...
        print(args.human_readable_names, args.input_log_files)
        assert len(args.human_readable_names) == len(args.input_log_files)
        assert len(args.colors) == len(args.input_log_files)

        for log, name, color, data_key in zip(args.input_log_files, args.human_readable_names, args.colors, args.data_key):
            scalars = extract_tensorboard_scalars(log, data_key)

            if data_key == "Eval_AverageReturn":
                scalars2 = extract_tensorboard_scalars(log, "Train_EnvstepsSoFar")
            else:
                scalars2 = None

            if not args.plot_mean_std:
                plot_scalars(plt.gca(), scalars, data_key, name, color, args.title, extra = scalars2)
